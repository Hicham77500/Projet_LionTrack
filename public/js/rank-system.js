...
/**
 * Module de gestion des grades et récompenses
 * Thème : Système militaire
 */

const RankSystem = (function() {
    // Les différents grades militaires et leurs seuils
    const RANKS = [
        { name: "Recrue", threshold: 0, icon: "https://cdn-icons-png.flaticon.com/512/9241/9241241.png" },
        { name: "Soldat", threshold: 2, icon: "https://cdn-icons-png.flaticon.com/512/9241/9241255.png" },
        { name: "Caporal", threshold: 5, icon: "https://cdn-icons-png.flaticon.com/512/9241/9241247.png" },
        { name: "Sergent", threshold: 10, icon: "https://cdn-icons-png.flaticon.com/512/9241/9241237.png" },
        { name: "Lieutenant", threshold: 15, icon: "https://cdn-icons-png.flaticon.com/512/9241/9241233.png" },
        { name: "Capitaine", threshold: 20, icon: "https://cdn-icons-png.flaticon.com/512/9241/9241203.png" },
        { name: "Commandant", threshold: 30, icon: "https://cdn-icons-png.flaticon.com/512/9241/9241201.png" },
        { name: "Colonel", threshold: 40, icon: "https://cdn-icons-png.flaticon.com/512/9241/9241199.png" },
        { name: "Général", threshold: 50, icon: "https://cdn-icons-png.flaticon.com/512/9241/9241195.png" }
    ];

    // Calcule le grade actuel en fonction du nombre de défis complétés
    function calculateRank(completedChallenges) {
        let currentRank = RANKS[0]; // Par défaut: Recrue
        
        for (let rank of RANKS) {
            if (completedChallenges >= rank.threshold) {
                currentRank = rank;
            } else {
                break;
            }
    ...
    // Récupère le grade actuel (utilisé pour l'affichage du profil)
    function getCurrentRank() {
        // Récupérer le nombre de défis complétés depuis le stockage local ou par défaut 20 (Capitaine)
        const completedChallenges = parseInt(localStorage.getItem('completedChallenges')) || 20;
        return calculateRank(completedChallenges);
    }

    // Met à jour l'affichage du grade dans l'interface
    function updateRankDisplay(completedChallenges) {
        const rank = calculateRank(completedChallenges);
        const rankName = document.querySelector('.profile-rank span');
        const rankIcon = document.querySelector('.rank-insignia');
        
        if (rankName && rankIcon) {
            rankName.textContent = rank.name;
            rankIcon.src = rank.icon;
            
            // Animation pour célébrer une promotion
            const prevRank = localStorage.getItem('userRank');
            if (prevRank && prevRank !== rank.name) {
                celebratePromotion(rank.name);
            }
    ...
    // Célèbre une promotion avec une notification spéciale
    function celebratePromotion(newRank) {
        if (window.AuthUI && AuthUI.showNotification) {
            AuthUI.showNotification('success', `🎖️ PROMOTION! Vous êtes maintenant ${newRank}! 🎖️`);
        }
    }
